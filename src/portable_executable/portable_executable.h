...
void getPeHeader(uint8_t* pFile, DosHeader* pDosHeader, PeHeader* pDest) {
  memcpy(pDest, pFile + pDosHeader->e_lfanew + sizeof(uint32_t),
         sizeof(PeHeader));
}

void getPeOptionalHeader(uint8_t* pFile, DosHeader* pDosHeader,
                         PeOptionalHeader* pDest) {
  memcpy(pDest,
         pFile + pDosHeader->e_lfanew + sizeof(uint32_t) + sizeof(PeHeader),
...
PeHeaderCollection getHeaders(uint8_t* pFile) {
  ...
  // PE Optional Header
  memcpy(&collection.opHeader, pFile + opHeaderOffset,
         sizeof(PeOptionalHeader));
  ...
  for (uint16_t i = 0; i < numSections; i++) {
    uint32_t sectionHeaderOffset =
        firstSectionHeaderOffset + i * sizeof(PeSectionHeader);
    memcpy(&collection.sectionHeaders[i], pFile + sectionHeaderOffset,
